...
import {
  updateTable,
  groupEventsByDate,
  formatGroupedEventsDescending,
} from "../ui/layout.js";
import {
  splitDateTimeIntoDateAndTime,
  convertToDateTime,
...
export function editEvent(
  ...
  const calendarList = screen.children.find(
    (child) => child.options.label === "Calendar List"
  );
  ...
  const eventTable = screen.children.find(
    (child) => child.options.label === "Current Events"
  );
  const editCommandList = screen.children.find(
    (child) => child.options.label === "Edit List"
  );
  const eventDetailTable = screen.children.find(
    (child) => child.options.label === "Event Details"
  );
  const { formBox, formFields } = createAddForm(screen);
  const tempFilePath = path.join(os.tmpdir(), "blessed-editor.txt");
  // selectedEventは既にEventオブジェクトとして渡される
  ...
  const { date: endDate, time: endTime } = splitDateTimeIntoDateAndTime(
    selectedEvent.end
  );
    ...
    switch (index) {
      case 0:
        ...
        calendarList.once("select", (item, index) => {
          ...
          calendarList.hide();
          formBox.setLabel(
            `Edit Event - ${selectedEditCalendar}  (Ctrl+S to save)`
          );
          formBox.show();
          formBox.focus();
          screen.render();
          ...
          formBox.focus();
          screen.render();
          const { date: sDate, time: sTime } = splitDateTimeIntoDateAndTime(
            originEvent.start
          );
          const { date: eDate, time: eTime } = splitDateTimeIntoDateAndTime(
            originEvent.end
          );
